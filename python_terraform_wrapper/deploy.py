...
def main(username,password,aws_access_key,aws_secret_key,aws_region,ec2_key_pair):
    ...
    return_code1, stdout, stderr = tf.apply(var= WebInDeploy_vars, capture_output=False, skip_plan=True, **kwargs)
    # return_code1 = 0
    ...
    vars = WebInDeploy_vars.update({'alb_arn': nlbDns,'int-nlb-fqdn': nlbDns} )
    if run_plan:
        tf.plan(capture_output=False, var=vars, **kwargs)
    return_code3, stdout, stderr = tf.apply(capture_output=False, skip_plan=True,
    ...
    return_code2, stdout, stderr = tf.apply(capture_output=False, skip_plan=True,
    ...
    tf = Terraform(working_dir='./WebInDeploy')
    albDns = tf.output('ALB-DNS')
    logger.info('Jenkins server DNS is {}'.format(albDns))
